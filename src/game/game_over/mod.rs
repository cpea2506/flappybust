...
fn spawn_game_over(
    ...
    mut score_ranks: Query<&mut Visibility, With<ScoreRank>>,
) {
    for mut visibility in &mut score_ranks {
...
fn move_scoreboard_up(
    ...
    mut hight_score_text: Query<&mut Visibility, (With<HighScoreText>, Without<ScoreText>)>,
    ...
    if transform.translation.y == 57f32 {
        let mut score_text_visibility = score_text.single_mut();
        let mut high_score_text_visibility = hight_score_text.single_mut();

        *score_text_visibility = Visibility::Visible;
...
fn scale_medal(
    ...
    if scoreboard_displayed.is_empty() || death_event.is_empty() || game_over_text_displayed.is_empty() {
        return;
    }
